...
1. **End-to-End Channel Stream**: For linear layers and convolutions of dimension $D$, freezes and zeroes out the tail channels in $[K, D)$ while keeping the first $K = \max(1, \lfloor D \times r_{\mathrm{stream}} \rfloor)$ channels active (configured via `stream_ratio`), letting computation reach cleanly end-to-end.
...
* **Loss Trajectory & Fit Verification**: Tracks initial loss $\mathcal{L}(0)$ vs. final loss $\mathcal{L}(T)$ on the bootstrap subset:
